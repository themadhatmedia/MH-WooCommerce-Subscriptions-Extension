<?php
/**
 * Plugin Name: WooCommerce Subscriptions Extension
 * Plugin URI: https://madhatmafia.com/
 * Description: Auto add subscriptions when email is already register for not login user
 * Version: 1.0
...
/* DASHBOARD PAGE */
//
add_action('admin_menu', 'mhm_wc_subscription_menu_setup');

function mhm_wc_subscription_menu_setup() {

		add_menu_page('MHM Woocommerce Membership Extension', 'MHM Woocommerce Membership Extension', 'manage_options', 'mhm-wc-membershipp-setup');

		add_submenu_page( 'mhm-wc-membershipp-setup', 'License', 'License',
		'manage_options', 'mhm-wc-membershipp-setup', 'mhm_wc_membership_setup_callback');

}



function mhm_wc_membership_setup_callback() {
	if ( isset($_POST['mhm_verify_wp_membership_form']) ) {
		update_option('mhm_memberpress_init_license_wc_membership_page_extention_email', $_POST['email'] );
		update_option('mhm_memberpress_init_license_wc_membership_page_extention_license_key', $_POST['license_key'] );
		update_option('mhm_memberpress_init_license_wc_membership_page_extention_product_id', 'wc-subscriptions' );
	...
	$email = get_option('mhm_memberpress_init_license_wc_membership_page_extention_email');
	$license_key = get_option('mhm_memberpress_init_license_wc_membership_page_extention_license_key');
...
if ( isset($verify) ) {
	...
	class WC_Checkout {
		...
		protected function process_customer( $data ) {
			...
			if ( ! is_user_logged_in() && ( $this->is_registration_required() || ! empty( $data['createaccount'] ) ) ) {
				...
				$customer_id = wc_create_new_customer_custom( $data['billing_email'], $username, $password );
	...
	if (  !function_exists( 'wc_create_new_customer_custom' ) ) {
		 ...
		 */
		function wc_create_new_customer_custom( $email, $username = '', $password = '' ) {
